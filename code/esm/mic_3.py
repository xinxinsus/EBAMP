...
def micRunmodel(trainfilename,testfilename):
    X_test, y_test, name_test = read_our(testfilename)
    X_train, y_train, name_test1 = read_our(trainfilename)
    clf = xgb.XGBClassifier(scale_pos_weight = 12.16,random_state = 0, n_jobs = -1)
...
def esmMicRunmodelReg(trainfilename,testfilename):
    X_train, y_train, name_test1 = read_our(trainfilename)

    sheet = pd.read_excel('/home/dell/Documents/GPTforBio/preprocessing/micpos.xlsx', 'res')
    validList = sheet.values
    ids = validList[:, 0]
    ...
    writer = pd.ExcelWriter('esm_pre_micvalues.xlsx')
    data.to_excel(writer, 'res', float_format='%.3f')
    writer.save()
    writer.close()

#esmMicRunmodelReg('/home/dell/Documents/GPTforBio/esm/esm_mic_train.xlsx',
#                                                '/home/dell/Documents/GPTforBio/esm/esm_mic_test.xlsx')